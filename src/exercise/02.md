...
Instead of overcomplicating components to handle any use case, we want to build
in a way that is flexible enough. We want control over the presentation of the
UI.

To share the state between the components implicitly we hold the state in the
parent component and the map the children and clone each one with the additional
properties:
`React.Children.map(children, child => React.cloneElement(child, {props}))`

Notice - with this method, we will get an error if we use DOM elements as 
children, so we need to check the child type and handle a case where it is 
not a function component.

If we want to allow only certain children types we can check if child.type 
is included in a list of allowed components.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=Advanced%20React%20Patterns%20%F0%9F%A4%AF&e=02%3A%20Compound%20Components&em=netanella1%40gmail.com).
